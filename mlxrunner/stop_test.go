...
func feedStops(stops []string, chunks ...string) (emitted string, stopped bool, tail string) {
	d := detokenizer{stops: stops}
	for _, c := range chunks {
		d.buf.WriteString(c)
		emit, stop := d.flushReady()
		emitted += emit
		if stop {
			return emitted, true, ""
		}
...
func TestFlushReadyPreservesIncompleteUTF8AndStopPrefix(t *testing.T) {
	d := detokenizer{stops: []string{"こん"}}

	d.buf.Write([]byte{0xe3, 0x81})
	if emit, stop := d.flushReady(); emit != "" || stop {
		t.Fatalf("first flush = (%q, %t), want empty without stop", emit, stop)
	}

	d.buf.Write([]byte{0x93, 0xe3})
	if emit, stop := d.flushReady(); emit != "" || stop {
		t.Fatalf("second flush = (%q, %t), want empty without stop", emit, stop)
	}

	d.buf.Write([]byte{0x82, 0x93})
	if emit, stop := d.flushReady(); emit != "" || !stop {
		t.Fatalf("third flush = (%q, %t), want empty with stop", emit, stop)
	}
}

func TestApplyStops(t *testing.T) {
	t.Run("whole in one chunk", func(t *testing.T) {
		emitted, stopped, _ := feedStops([]string{"STOP"}, "hello STOP world")
	...
	t.Run("split across chunks", func(t *testing.T) {
		emitted, stopped, _ := feedStops([]string{"STOP"}, "hello ST", "OP world")
		if !stopped || emitted != "hello " {
			t.Fatalf("got (%q, stop=%t), want (%q, true)", emitted, stopped, "hello ")
		}
	})
	...
	t.Run("split one byte at a time", func(t *testing.T) {
		emitted, stopped, _ := feedStops([]string{"STOP"}, "a", "S", "T", "O", "P", "b")
	...
	t.Run("stop at start", func(t *testing.T) {
		emitted, stopped, _ := feedStops([]string{"STOP"}, "STOP after")
		if !stopped || emitted != "" {
	...
	t.Run("partial that never completes is flushed", func(t *testing.T) {
		emitted, stopped, tail := feedStops([]string{"STOP"}, "done ST")
	...
	t.Run("no stops emits everything unchanged", func(t *testing.T) {
		emitted, stopped, tail := feedStops(nil, "hello STOP world")
